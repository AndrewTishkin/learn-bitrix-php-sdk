# Офлайн-события Битрикс24: полное руководство по синхронизации
...
Фильтрация по воронке делается **в вашем коде** при обработке событий (подробнее — в Шаге 3).

### Код: `install.php` с регистрацией событий

После сохранения токенов нужно зарегистрировать нужные события. Удобно делать прямо в `install.php`:
...
Поэтому узнать `CATEGORY_ID` (воронку) можно только запросив сделку через `crm.deal.list` с фильтром. Именно там и применяется фильтр по воронке.
...
│     → crm.deal.list с CATEGORY_ID + >=DATE_MODIFY      │
│     → если сделка в нужной воронке — синхронизировать  │
│     → если нет — подтверждаем без обработки             │
│                                                         │
...
$response = $b24->core->call('event.offline.get', [
    'clear'          => 0,   // не удалять сразу — нужен process_id для подтверждения
    'auth_connector' => 'my_sync',
    'limit'          => 50,
]);
...
foreach ($events as $event) {
    ...
    // EVENT_DATA = false — это нормально! Данных нет, только факт изменения.
...
// Фильтрация по воронке происходит здесь — в запросе к API, не в подписке
...
Если нужно реагировать именно на **перемещение сделки в нужную воронку**, есть отдельное событие:
...
Это событие срабатывает только при смене воронки, а не при любом обновлении. Воронку назначения всё равно проверяете через `crm.deal.list` с `CATEGORY_ID`.
...
```php
// При регистрации подписки (install.php):
...
Типизированные методы SDK (`$b24->getCRMScope()->deal()->update(...)`) параметр `auth_connector` не поддерживают — он не заложен в их сигнатуру. Для всех модифицирующих запросов, где нужен `auth_connector`, используйте `$b24->core->call()` напрямую.
...
- [ ] (Опционально, Pro тариф) Передача `auth_connector` во все модифицирующие запросы через `core->call()`
